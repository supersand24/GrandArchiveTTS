...
local playerVisibilities = {
  mySide = { visible = "Blue|Yellow|Grey|Black", invisible = "Blue|Grey|Black"},
  oppositeSide = { visible = "Red|Green", invisible = "Pink"},
}
  ...
  WebRequest.get(deckURL, function(request)
    if request.is_error then
      ...
      if json.error ~= nil then
        ...
        --Shuffle Main Deck
        local mainDeckCount = 0
        for c,card in pairs(json.cards.main) do
          mainDeckCount = mainDeckCount + card.quantity
        end
  ...
  myPlayer.showConfirmDialog("Reset your placespace?",
    function (player_color)
      for _, obj in pairs(fieldZone.getObjects()) do
        if obj.type == "Card" or obj.type == "Deck" or obj.type == "Block" or obj.type == "Dice" then
          destroyObject(obj)
        end
      end
      ...
      for _, obj in pairs(myPlayer.getHandObjects()) do
        destroyObject(obj)
      end
